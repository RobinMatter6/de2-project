...
if top_repos_raw:
    # Get current time to calculate repository age
    ...
    repo_data_list = [{
        ...
        'repo_age_days': (current_time - datetime.strptime(repo['created_at'], "%Y-%m-%dT%H:%M:%SZ")).days
    } for repo in top_repos_raw]
    
    df_repos = pd.DataFrame(repo_data_list)
    
    base_csv_path = os.path.join(DATA_DIR, 'top_1000_github_repos.csv')
    df_repos.to_csv(base_csv_path, index=False)
    print(f"\n Base data successfully saved to {base_csv_path}")
...
final_csv_path = os.path.join(DATA_DIR, 'top_1000_github_repos_with_commits.csv')
df_repos.to_csv(final_csv_path, index=False)

print(f"\n Feature extraction completed successfully. Final dataset saved to {final_csv_path}")
print(df_repos.head())